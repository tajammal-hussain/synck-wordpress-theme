 ...
 */
if ( ! function_exists( 'Synck_kirki_update_url' ) ) {
	function Synck_kirki_update_url( $config ) {
		$config['url_path'] = get_template_directory_uri() . '/inc/admin/kirki/';

		return $config;
	}
}
add_filter( 'kirki_config', 'Synck_kirki_update_url' );
/**
 ...
 */
function Synck_kirki_modules( $modules ) {
...
add_filter( 'kirki_modules', 'Synck_kirki_modules' );

/**
 * Custom option sanitize callback.
 */
function synck_custom_sanitize( $content ) {
	return $content;
}
 ...
 */
function synck_parse_font_family( $family ) {
	if ( ! is_string( $family ) ) {
		return '';
	}
...
Synck_Option::add_config( 'option', array(
